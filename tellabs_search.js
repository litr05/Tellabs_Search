...
function search_node(node) {
    ...
    for (var i = 0; i < map.length; i++){var str_node=map[i].getAttribute('onmouseover');
...
function my(p1) {
    ...
    if (p1!=""){
        ...
        if (id!=undefined){
            var coords = id.coords;var div_image = document.getElementById('wmapimage');var newDiv = document.createElement('canvas');newDiv.id='canv';
            ...
            var elements = document.getElementsByTagName('div');elements[0].appendChild(newDiv);
            ...
            c.style.width=x2-x1-1+'px';c.style.height=y2-y1-1+'px';
            c.style.top = (-h-9)+y1+'px';c.style.left = x1-4+'px';
