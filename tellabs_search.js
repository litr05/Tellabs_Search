...
function search_node(node,map){var mapa;if(map==undefined)mapa=$("area[id^='NODE']");else mapa=map;var node_id="";var node_str=mbh_node(node);
    for(var i=0;i<mapa.length;i++){var str_node=mapa[i].getAttribute('onmouseover');
        if(~str_node.indexOf(node_str)){node_id = mapa[i];break;}}if(node_id!="")return node_id;else return undefined;}
...
function draw_node(id){var coords = id.coords;var div_image = document.getElementById('wmapimage');var newDiv = document.createElement('canvas');newDiv.id='canv';
    ...
    color_node(c);}
function my(p1){del_canvas();if(p1!=""){var id=search_node(p1);
        if (id!=undefined) draw_node(id);else{alert("ничего не найдено, скорректируйте запрос\nxx-xxx_ - площадка 5 обязательных цифр")}}
